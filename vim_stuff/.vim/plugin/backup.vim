...
if exists("s:loaded_backup") || version < 700
    ...
    if !exists("g:backup_separator")
        let g:backup_separator="."
    endif
	...
	"
	    ...
	    let l:Existing_Backups = sort (
		   \ split (
		   \ glob (l:Backup_Path . '/' . a:Doc_Name . g:backup_separator . '*~'), "\n"))
	    let &backupext=g:backup_separator . strftime("%Y%m%d_%H%M%S") . '~'
