...
" Rspec
"=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
inoremap <buffer><localleader>it <c-r>=IMAP_PutTextWithMovement("it \"<+ description +>\" do\n<+ block +>\nend")<cr>
inoremap <buffer><localleader>sd <c-r>=IMAP_PutTextWithMovement("should ")<cr>
inoremap <buffer><localleader>sn <c-r>=IMAP_PutTextWithMovement("should_not ")<cr>
inoremap <buffer><localleader>se<tab> <c-r>=IMAP_PutTextWithMovement("should == ")<cr>
inoremap <buffer><localleader>bn <c-r>=IMAP_PutTextWithMovement("be_nil")<cr>
inoremap <buffer><localleader>bm <c-r>=IMAP_PutTextWithMovement("be_empty")<cr>
" inoremap <buffer><localleader>xx <c-r>=IMAP_PutTextWithMovement("")<cr>


"=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
" Add snippets
"=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
inoremap <buffer><localleader>ata <c-r>=IMAP_PutTextWithMovement("attr_accessor :<+ accessor name +>")<cr>


"=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
" Replace the default mapping(just add a <tab> after the default one)
"=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
inoremap <buffer><localleader>bl<tab> <c-r>=IMAP_PutTextWithMovement("{ \|<+ arg +>\| <+ block +> }")<cr>
inoremap <buffer><localleader>def<tab> <c-r>=IMAP_PutTextWithMovement("def <+ defname +>\nend")<cr>
inoremap <buffer><localleader>do<tab> <c-r>=IMAP_PutTextWithMovement("do \|<++>\|\n<++>\nend")<cr>
